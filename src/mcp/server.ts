...
function registerGameTools() {
  ...
  server.tool("getGameState", "📊 Get current game state", {}, async () => {
  ...
  server.tool(
    ...
    async ({ playerId }) => {
      ...
      const handCards = player.hand.map((cardId) => {
        ...
        return card
          ? `🎴 ${card.name} (${card.attack}/${card.defense}/${card.charm}) - ${card.cost} mana - ${card.rarity}`
          : `❓ Unknown card: ${cardId}`;
      });
  ...
  server.tool(
    ...
    async () => {
      ...
      // Player 1 field
      fieldInfo += `🎮 ${gameState.players.player1.name}'s Field (${gameState.players.player1.field.length}/5):\n`;
      if (gameState.players.player1.field.length === 0) {
        ...
        gameState.players.player1.field.forEach((waifu, index) => {
          ...
          const statusIcon = waifu.canAttack
            ? "⚡ Ready"
            : "😴 Summoning Sickness";
          fieldInfo += `  [${index}] ${card?.name || "Unknown"} (${
            waifu.currentAttack
          }/${waifu.currentDefense}/${waifu.currentCharm}) ${statusIcon}\n`;
      ...
      // Player 2 field
      fieldInfo += `🎮 ${gameState.players.player2.name}'s Field (${gameState.players.player2.field.length}/5):\n`;
  ...
  server.tool(
    ...
    async ({ playerId, cardId }) => {
      ...
      const summonResult = WaifuGameEngine.summonWaifu(
        gameState,
          ...
          content: [
            {
              ...
              text:
                `❌ Cannot summon waifu!\n\n${summonResult.error}\n\n` +
                `📊 Field Status: ${summonResult.waifusOnField}/${summonResult.maxFieldSize} waifus`,
            },
          ],
        };
      }
        ...
        content: [
          {
            type: "text",
            text:
              `✅ ${player.name} successfully summoned ${card?.name}!\n\n` +
              `⚔️ Stats: ${card?.attack}/${card?.defense}/${card?.charm}\n` +
              `💎 Mana remaining: ${player.mana}/${player.maxMana}\n` +
              `🃏 Hand: ${player.hand.length} cards\n` +
              `⚔️ Field: ${summonResult.waifusOnField}/${summonResult.maxFieldSize} waifus\n` +
              `🆔 Instance ID: ${summonResult.instanceId}`,
          },
        ],
      };
    }
  ...
  server.tool(
    ...
    async ({ playerId }) => {
      ...
      const success = WaifuGameEngine.advancePhase(
        gameState,
        playerId as PlayerId
      );
  ...
  server.tool(
    ...
    "⚔️ Attack opponent or their waifu",
    {
      ...
      attackerIndex: z
        .number()
        .describe("Index of attacking waifu on field (0-based)"),
      targetType: z.enum(["player", "waifu"]).describe("Attack target type"),
      targetIndex: z
        .number()
        .optional()
        .describe("Index of target waifu (if attacking waifu)"),
    },
...
async function main() {
  ...
  console.log(
    "💡 Game logic and rules are enforced, but AI makes all decisions"
  );
