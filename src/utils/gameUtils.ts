...
// Battle calculations with improved combat mechanics
...
export function calculateBattle(
  ...
  if (attackerAttack > defenderDefense) {
    ...
    description = `${attackerName} (${attackerAttack} ATK) defeats ${defenderName} (${defenderDefense} DEF). ${damage} damage dealt to opponent.`;
    ...
    const damage = defenderDefense - attackerAttack;
    description = `${defenderName} (${defenderDefense} DEF) defeats ${attackerName} (${attackerAttack} ATK). ${damage} damage dealt to attacker's owner.`;
    ...
    description = `${attackerName} (${attackerAttack} ATK) and ${defenderName} (${defenderDefense} DEF) destroy each other in battle!`;
...
export function validateManaForCard(
  gameState: GameState,
  playerId: PlayerId,
  cardCost: number
): boolean {
...
export function validateFieldSpace(
  gameState: GameState,
  playerId: PlayerId
): boolean {
